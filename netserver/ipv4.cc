...
void Netserver_IPv4::send(NetserverPacket& p, const std::vector<iovec>& iovs_in, size_t iovlen, int current) const
{
	...
	while (iter != iovs.end()) {
		...
		if (chunk > max_frag) {
			...
			send_fragment(p, current, offset, chunk, iovs, iovs.size(), false);
